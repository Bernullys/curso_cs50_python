...
class Bill:                                 # With this class I'm creating customers by name and adding the items to their order list.
    ...
    def order(self, items):                 # This method will be used every time a instancie oreder an item. Items has to be in the menu.
        ...
        with open ("./current_bills.csv", "a", newline="") as file:     # This part is to save every order into a csv file and keep register.
    ...
    def current_orders(self):                   # This method is to check the current tap of each instance.
        if len(self.items) > 0:
            print(f"{self.customer_name} these are your current orders:")
            for item in self.items:
                print(f"{item} ${menu[item]:,.2f}")
        else:
            print(f"{self.customer_name} you don't have any orders.")
    ...
    def invoice(self, customer, tip_percentage):
        ...
        invoice_pdf.cell(0, 0, f"Subtotal: ${invoice_amount:,.2f}", align="R")
        invoice_pdf.ln(7)
